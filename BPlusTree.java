...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (!op.isPresent())
            return;
        List<DataBox>rootkeys = new ArrayList<DataBox>();
        rootkeys.add(op.getFirst());
        List<Long>rootchildren = new ArrayList<Long>();
        rootchildren.add(root.getPage().getPageNum());
        rootchildren.add(op.getSecond());
        InnerNode newRoot = new InnerNode(metadata, bufferManager, rootkeys, rootchildren, treeContext);
